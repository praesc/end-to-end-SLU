...
class Model(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.pretraining_type != 0:
            ...
            if self.is_cuda:
                ...
                pretrained_model.to(torch.device('cuda'))
            else:
        ...
        if not self.seq2seq:
            ...
            #self.intent_layers.cuda()
            #print("hereee", self.intent_layers.is_cuda)
    ...
    def get_intent_label(self, predicted_intent):
        intent = []
        for idx, slot in enumerate(self.Sy_intent):
            for value in self.Sy_intent[slot]:
                if predicted_intent[idx].item() == self.Sy_intent[slot][value]:
                    intent.append(value)
